...
*/
   /* const values = [
      name, prompt, Age, gender, theme, reason,...finalResponse["diaryEntries"].flatMap(entry => [entry.Entry, entry.Symptoms.join(", ")])
    ];
...
function Check(name, prompt, Age, gender, theme, reason, newResponse,oldResponce) {
  ...
  if (Og_ans.length === 15) {
    ...
    const finalResponse = oldResponce["diaryEntries"].map(obj=>
      ...
      Marathi_prompt(name, prompt, Age, gender, theme, reason,finalResponse,Og_ans,oldResponce)
      //console.log(finalResponse)
...
Ensure:
- Symptoms should be strictly from above only
...
//${console.log(JSON.stringify(arr.map(x => finalResponce[x])))}
async function Marathi_prompt(name, prompt, Age, gender, theme, reason,finalResponce,arr,oldResponce){
 const Mara_prompt =` Translate the given English text into Marathi(in Devnagri script) in a conversational and natural tone, keeping the context and emotions intact. Use a mix of Marathi and English, where commonly used English words (like study, exam, singing, pressure, etc.) are retained instead of being translated into Marathi. Ensure the final output sounds authentic and relatable, as if written by a person fluent in Marathi but comfortable using English terms where necessary.
  ${JSON.stringify(arr.map(x => finalResponce[x]))}${console.log(JSON.stringify(arr.map(x => finalResponce[x])))}
  Provide the response in JSON format as before.keep day number as it is {
  "diaryEntries": [
    { Day : "keep it as it is", "Entry": "Text...", "Symptoms": ["symptom1", "symptom2"] },
  ], and keep common words in marathi`
  try {
    const result3 = await model.generateContent(Mara_prompt);
    let Response3 = result3.response.text().replace(/^```|json\n|\n```|``` $/g, '');
    //console.log(Response3)
    Response3 = JSON.parse(Response3);
    console.log("Marathi-prompting AI...");
    if(Og_ans == 15){
    //  Append(name, prompt, Age, gender, theme, reason, Response3);
      
    }else{

      Check(name, prompt, Age, gender, theme, reason, Response3,oldResponce)
    } 
